...
# ╔═╡ b8db43f0-f6d1-11ef-1027-ef2983ea76c8
using ElectroPhysiology, PhysiologyAnalysis, PhysiologyModeling

# ╔═╡ 50c85af3-70a0-45fc-a487-7d20cd7caeaa
using Statistics, Optim

# ╔═╡ 1397559e-e87d-4565-bd74-b03b93a376a2
using GLMakie, PhysiologyPlotting

# ╔═╡ 39ca49f6-a9b0-4e8d-96eb-d6da4bcdd713
using DataFrames, CSV, PlutoUI

# ╔═╡ 8f78a3b5-c330-4b16-91e4-a9b5796de675
include(raw"C:\Users\mtarc\Julia\dev\PhysiologyModeling.jl\RetinaDeepModel\OpenData.jl")

# ╔═╡ ea461b21-f990-4979-9d9a-fa1702f65c33
begin
	include(raw"C:\Users\mtarc\Julia\dev\PhysiologyModeling.jl\RetinaDeepModel\Models.jl")
...
begin #Open all the data
	...
	photon_flux = 400.0
	Stim(t) = stim_start <= t <= stim_end ? photon_flux : 0.0
	println("Stimulus runs from $(stim_start) to $(stim_end)")

end;

# ╔═╡ c4423638-321a-428b-b6bc-c45726af60f5
md"""
### Open the parameters and bounds
"""

# ╔═╡ 31817ea0-413b-4138-ad16-f8adeb88f1b9
md"""

TP1 = $(@bind tP1 PlutoUI.Slider(lower_bounds[1]:0.001:upper_bounds[1], show_value = true, default = p0[1]))

TauH = $(@bind tH PlutoUI.Slider(0.0:0.1:100.0, show_value = true, default = 75.0))

L2 = $(@bind l1 PlutoUI.Slider(0.01:0.01:10, show_value = true, default = 0.25))

H1 = $(@bind h1 PlutoUI.Slider(0.001:0.001:1, show_value = true, default = 0.006))
"""

# ╔═╡ 4a69478f-022c-4ccb-a12c-a88ed288080a
begin
	opt_params = deepcopy(p0)
	opt_params[1] = tP1
	opt_params[7] = l1
	opt_params[8] = h1
	opt_params[29] = tH
...
# ╔═╡ 6f5bb062-9915-4c76-8a94-7d9b14ce1dd9
md"""
### Now write the optimization problem

"""

# ╔═╡ d8b0d54b-9f29-4bd0-9539-fb44c942cb52
begin
	sol_a, ERGa_t = simulate_awave(a_dataERG, opt_params);
	sol_t = sol_a.t
	# Plot the a-wave solution and relevant variables
	fig_a = Figure()
	ax1_a = Axis(fig_a[1,1])
	experimentplot!(ax1_a, a_dataERG, channel = 3)
	lines!(ax1_a, sol_t, ERGa_t, color = :red, label = "Simulated ERG")
	
	ax2_a = Axis(fig_a[2,1])
	lines!(ax2_a, sol_t, ERGa_t, color = :red, label = "Simulated ERG")
	# #Extract model components
	# Rt = map(t -> sol_a(t)[1], sol_t)
	# Tt = map(t -> sol_a(t)[2], sol_t)
	# Pt = map(t -> sol_a(t)[3], sol_t)
	# Gt = map(t -> sol_a(t)[4], sol_t)
	 Jt = map(t -> sol_a(t)[5], sol_t)
	 Ht = map(t -> sol_a(t)[6], sol_t)
	# At = map(t -> sol_a(t)[7], sol_t)
	# Bt = map(t -> sol_a(t)[15], sol_t)
	
	# #Plot each component
	 ax3_a = Axis(fig_a[3,1])
	# #lines!(ax2_a, sol_t, Rt)
	# #lines!(ax2_a, sol_t, Tt)
	# #lines!(ax2_a, sol_t, Pt)
	# #lines!(ax2_a, sol_t, Gt)
	lines!(ax3_a, sol_t, Jt)
	lines!(ax3_a, sol_t, Ht)

	# ax4_a = Axis(fig_a[3,1])

	# lines!(ax4_a, sol_t, At)
	
	# linkxaxes!(ax1_a, ax2_a)
	# #xlims!(ax1_a, 0.0, 400.0)
	# #xlims!(ax2_a, 0.0, 400.0)
	fig_a
end

# ╔═╡ 86a09dfb-1f5b-4383-9525-8169550a9940
begin
	sol_ab, ERGab_t = simulate_abwave(ab_dataERG, opt_params);
		# Plot the a-wave solution and relevant variables
	fig_b = Figure()
	ax1_b = Axis(fig_b[1,1])
	experimentplot!(ax1_b, ab_dataERG, channel = 3)
	lines!(ax1_b, sol_t, ERGab_t, color = :red, label = "Simulated ERG")
	
	ax2_b = Axis(fig_b[2,1])
	lines!(ax2_b, sol_t, ERGab_t, color = :red, label = "Simulated ERG")
	
	#Extract model components
	Ot = map(t -> sol_ab(t)[8], sol_t)
	# #Plot each component
	# ax3_b = Axis(fig_b[3,1])
	# lines!(ax3_b, sol_t, Ot)

	fig_b
end

# ╔═╡ a397e43c-9fa1-43b1-8407-13dba50d5e36
sol_abm, ERGabm_t = simulate_model(abm_dataERG, opt_params);

# ╔═╡ 9c84272c-1099-4e72-8b05-ac9e68f74c85
begin
fig = Figure(size = (1000, 600))
	...
	lines!(ax1, sol_t, ERGa_t, color = :red, label = "Simulated ERG", alpha = 0.2)
	lines!(ax2, sol_t, ERGab_t, color = :red, label = "Simulated ERG", alpha = 0.2)
	lines!(ax3, sol_t, ERGabm_t, color = :red, label = "Simulated ERG", alpha = 0.2)
	lines!(ax1b, sol_t, ERGa_t, color = :red, label = "Simulated ERG")
	lines!(ax2b, sol_t, ERGab_t, color = :red, label = "Simulated ERG")
	lines!(ax3b, sol_t, ERGabm_t, color = :red, label = "Simulated ERG")
...
PhysiologyAnalysis = "69cbc4a0-077e-48a7-9b45-fa8b7014b5ca"
PhysiologyModeling = "6edd8d80-c083-4fb2-88a3-9ae8efe7fb75"
PhysiologyPlotting = "d5a2a81f-3167-4eb5-8f67-b1ebe6a4430d"
...
CSV = "~0.10.15"
DataFrames = "~1.7.0"
ElectroPhysiology = "~0.5.34"
GLMakie = "~0.11.3"
Optim = "~1.11.0"
PhysiologyAnalysis = "~0.6.31"
PhysiologyModeling = "~0.1.2"
PhysiologyPlotting = "~0.1.16"
PlutoUI = "~0.7.23"
Statistics = "~1.11.1"
"""
...
manifest_format = "2.0"
project_hash = "89e55d3d8b107b29578871d2b2a1650d826aded3"
...
deps = ["CurveFit", "DSP", "Dates", "FileIO", "ImageMagick", "Images", "OffsetArrays", "Polynomials", "Requires", "SparseArrays", "Statistics", "TimeZones"]
git-tree-sha1 = "d0f60bc4b969eff1e1b761e39a8f3053cfe4bd74"
...
deps = ["CSV", "Dates", "ElectroPhysiology", "FileIO", "HypothesisTests", "ImageFiltering", "ImageMagick", "Images", "LsqFit", "Peaks", "Polynomials", "Requires", "Statistics", "StatsBase"]
git-tree-sha1 = "8090999a023aa91e80015c07b224e40de3b871d2"
uuid = "69cbc4a0-077e-48a7-9b45-fa8b7014b5ca"
version = "0.6.31"

[[deps.PhysiologyModeling]]
deps = ["DiffEqCallbacks", "DifferentialEquations", "ElectroPhysiology", "ForwardDiff", "LinearAlgebra", "Logging", "NLsolve", "Requires", "SparseArrays", "TerminalLoggers"]
git-tree-sha1 = "bc5c9d1c92d75287fc91c2df09901bcbfb5370d1"
uuid = "6edd8d80-c083-4fb2-88a3-9ae8efe7fb75"
version = "0.1.2"

[[deps.PhysiologyPlotting]]
deps = ["Dates", "Distributions", "ElectroPhysiology", "Requires", "Statistics"]
git-tree-sha1 = "844d186a5c833b9762385eae22cbbb847e161981"
