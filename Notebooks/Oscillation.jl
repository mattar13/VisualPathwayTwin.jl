...
# ╔═╡ 15b1aa50-feb7-11ef-1f4f-6fd4dbdc4650
using DifferentialEquations, GLMakie

# ╔═╡ 2ebe3f0e-f253-4b81-91a9-4bad9a18487a
sigm(x, l, xh) = x > 0 ? (1/(1+exp(-l*(x-xh)))) : 0

# ╔═╡ 22d7173c-073b-410f-b754-a6a25ca8c816
begin
	stim_start = 100.0
	stim_end = 300.0
end

# ╔═╡ 8b867e89-ad90-4f67-ab9f-1e92d3e717fd
stim(t) = stim_start < t < stim_end ? 0.15 : 0.0

# ╔═╡ 00db7ad6-afd8-4064-a66a-55cd351c5596
function op_system!(du, u, p, t)
    B, O1, O2, O3, O = u
    k5, k6, k7, l, h, τO = p  # natural frequency, 
	du[1] = (20.0*stim(t) - B)/200.0
	du[2] = O2
	du[3] = (-k5*O1 - k6*O2)+sigm(B, l, h)
	du[4] = -k7*B - (O3/τO)
	du[5] = (O3 * O2) - O
	return nothing
end

# ╔═╡ c8a0dfd9-9c0c-4e16-a2e4-c0e42481c5d1
begin
	# Parameters tuned to match the image
	ω₀ = 0.05*3.14  # Higher frequency for faster oscillations
	ζ = 0.2   # Moderate damping
	...
	p = [k5, k6, k7, l, h, tO]
	tspan = (0.0, 1000.0)
	prob = ODEProblem(op_system!, u0, tspan, p);
	sol = solve(prob, Tsit5(), tstops = [stim_start, stim_end]);
end;

# ╔═╡ 87467bca-9607-4533-90d8-45504fa256ac
k5

# ╔═╡ d466c0f5-c78a-4dff-a793-9732ff1f1f50
k6

# ╔═╡ 7aa49705-5dd0-4900-9673-c804758356e7
begin
	sol_t = tspan[1]:1.0:tspan[end]
	
	fig = Figure()
	
	ax1 = Axis(fig[1,1]); hidespines!(ax1)
	lines!(ax1, sol_t, map(t -> stim(t), sol_t), color = :black, linewidth = 2)
	ax1.ylabel = "Photons (μV)"
	
	ax2 = Axis(fig[2,1]); hidespines!(ax2)
	lines!(ax2, sol_t, map(t -> sol(t)[1], sol_t), color = :black, linewidth = 2)
	ax2.ylabel = "b-wave (μV)"
	
	ax3 = Axis(fig[3,1]); hidespines!(ax3)
	lines!(ax3, sol_t, map(t -> sol(t)[5], sol_t), color = :black, linewidth = 2)
	ax3.ylabel = "OP voltage (μV)"
	
	ax4 = Axis(fig[4,1]); hidespines!(ax4)
	lines!(ax4, sol_t, map(t -> sol(t)[5]+sol(t)[1], sol_t), color = :black, linewidth = 2)
	ax4.xlabel = "Time (ms)"
	ax4.ylabel = "combined (μV)"
	#ax4 = Axis(fig[4,1])
	#lines!(ax4, sol_t, map(t -> sol(t)[4], sol_t))
	
	fig
	
end

# ╔═╡ ae22b180-c53e-4a4e-905b-f7bab405afd5
save("Oscillation.png", fig)

# ╔═╡ e515975f-d338-46de-9783-2547f35d9954
begin
	fig2 = Figure()
	
	ax12 = Axis(fig2[1,1]); hidespines!(ax12)
	lines!(ax12, sol_t, map(t -> stim(t), sol_t), color = :black, linewidth = 2)
	ax12.ylabel = "Photons (μV)"
	
	ax22 = Axis(fig2[2,1]); hidespines!(ax22)
	lines!(ax22, sol_t, map(t -> sol(t)[1], sol_t), color = :black, linewidth = 2)
	ax22.ylabel = "b-wave (μV)"
	
	ax32 = Axis(fig2[3,1]); hidespines!(ax32)
	lines!(ax32, sol_t, map(t -> sol(t)[5], sol_t), color = :black, linewidth = 2)

	ax42 = Axis(fig22[4,1]); hidespines!(ax4)
	lines!(ax42, sol_t, map(t -> sol(t)[4], sol_t))
	ax42.xlabel = "Time (ms)"
	ax42.ylabel = "OP voltage (μV)"
	fig2
end
...
manifest_format = "2.0"
project_hash = "46c9145fc953fcbf72e716d411f238756358fca1"
