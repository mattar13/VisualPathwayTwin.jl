...
# ╔═╡ 1871e33d-0016-4e70-848e-603e7698c150
using PlutoUI

# ╔═╡ de86ad00-0b2c-11f0-1076-c385afc46b39
using ElectroPhysiology

# ╔═╡ 8f2737c8-a788-416a-b962-bf8d6d5a449c
using DifferentialEquations

# ╔═╡ 16904194-57e1-463f-ba11-00745eb6c021
using ForwardDiff

# ╔═╡ fef68f94-1f71-4a12-93aa-19bb2b6e647a
using GLMakie, PhysiologyPlotting

# ╔═╡ f97a14e5-ac59-4550-ba01-78f413b00cd1
using DataFrames, CSV

# ╔═╡ 787a65f8-d9b8-43e3-a9e3-1ea6d5966990
begin
	include("../OpenData.jl")
...
# ╔═╡ 78af9771-abb3-469a-b9db-812acad51d4c
begin
	include("../Models.jl")
...
# ╔═╡ 800aa4f7-62bb-47e6-93f8-8fb2b103d790
begin
	stim_start = getStimulusStartTime(a1_dataERG)[1]*1000
	stim_end = getStimulusEndTime(a1_dataERG)[1]*1000
end

# ╔═╡ 36a6b048-102d-4264-8b07-caf2765351ce
md"""

R* - kP1 = $(@bind kp1 PlutoUI.Scrubbable(lower_bounds[1]:0.0001:upper_bounds[1], default = p0[1], format=".5f"))
T* - kP2 = $(@bind kp2 PlutoUI.Scrubbable(lower_bounds[2]:0.001:upper_bounds[2], default = p0[2], format=".5f"))
P* - kP3 = $(@bind kp3 PlutoUI.Scrubbable(lower_bounds[3]:0.1:upper_bounds[3], default = p0[3], format=".5f"))
G* kP4 = $(@bind kp4 PlutoUI.Scrubbable(lower_bounds[4]:0.1:upper_bounds[4], default = p0[4], format=".5f"))

kP5 = $(@bind kp5 PlutoUI.Scrubbable(lower_bounds[5]:0.1:upper_bounds[5], default = p0[5], format=".5f"))      
kP6 = $(@bind kp6 PlutoUI.Scrubbable(lower_bounds[6]:0.1:upper_bounds[6], default = p0[6], format=".5f"))

L1 = $(@bind l1 PlutoUI.Slider(lower_bounds[7]:0.001:upper_bounds[7], show_value = true, default = p0[7]))
H1 = $(@bind h1 PlutoUI.Slider(lower_bounds[8]:0.0001:upper_bounds[8], show_value = true, default = p0[8]))

JMAX = $(@bind jmax PlutoUI.Slider(lower_bounds[9]:0.001:upper_bounds[9], show_value = true, default = p0[9]))
HMAX = $(@bind hmax PlutoUI.Slider(lower_bounds[10]:0.001:upper_bounds[10], show_value = true, default = p0[10]))

tj= $(@bind tj PlutoUI.Slider(lower_bounds[15]:0.001:upper_bounds[15], show_value = true, default = p0[15]))
tH = $(@bind th PlutoUI.Slider(lower_bounds[16]:0.001:upper_bounds[16], show_value = true, default = p0[16]))
"""

# ╔═╡ 31d0d10e-6346-4d57-85a3-511e38ecb503
begin
...
# ╔═╡ d89fde1f-a6c5-499f-9f30-81e94d14123a
begin
	loss_fn(p) = loss_graded(data_dict, p, 
							 stim_start = stim_start, stim_end = stim_end)[2]
	grad = ForwardDiff.jacobian(loss_fn, opt_params)[:, 1:16]
end;

# ╔═╡ 0125cede-a3a0-4998-92f3-3cbf2e5f97a6
#Run the simulation
begin
	sim = []
...
# ╔═╡ fad92800-e459-4668-9032-25c86f560c64
begin
	...
	hidespines!(ax1)
	ax2 = Axis(fig[1,2], title = "Simulation")
	hidespines!(ax2)
	ax3 = Axis(fig[1,3], title = "Weighted MSE")
	hidespines!(ax3)
	
	# ax1a = Axis(fig[1, 1], title = "Rhodopsin"); 
	# hidespines!(ax1a)
	# ax1b = Axis(fig[1, 2], title = "Transducin"); 
	# hidespines!(ax1b)
	# ax1c = Axis(fig[2, 1], title = "PDE"); 
	# hidespines!(ax1c)
	# ax1d = Axis(fig[2, 2], title = "cGMP"); 
	# hidespines!(ax1d)


	ax2a = Axis(fig[2,1], title = "Jt")
	hidespines!(ax2a)
	ax2b = Axis(fig[2,3], title = "Ht")
	hidespines!(ax2b)
	...
	mins = zeros(length(sim))
	data_mins = zeros(length(sim))
	for (i, sol) in enumerate(sim)
	 ...
	 #    )
		mins[i] = abs(minimum(a_wave))
		data_mins[i] = abs(minimum(exper))
		#scatter!(ax3, log10(photons[i]), -minimum(a_wave))
	...
	lines!(ax3, log10.(photons), mins)
	scatter!(ax3, log10.(photons), mins)
	lines!(ax3, log10.(photons), data_mins)
	scatter!(ax3, log10.(photons), data_mins)
	
	fig
end


# ╔═╡ fb8fc5df-5b7a-49d1-af28-00be48ebc20f
begin
	#Make positions every 4
	pos = 1:4:16*4 |> collect
	figA, ax = barplot(pos, sum(grad[:, 1:16], dims = 1)[1,:])
	scatter!(ax, pos, grad[1, 1:16])
	scatter!(ax, pos.+1, grad[2, 1:16])
	scatter!(ax, pos.+2, grad[3, 1:16])
	scatter!(ax, pos.+3, grad[4, 1:16])
	figA
end

# ╔═╡ 756a35d5-1d72-4bd5-abf2-fcec63a2b3d6
begin
	j_rng = collect(-1.0:0.01:0.1)
	h_rng = map(j -> H_inf(j, l1, h1), j_rng)
	lines(j_rng, h_rng)
end
...
manifest_format = "2.0"
project_hash = "5d5482b4aefc39868e7c691da54980a95374dc71"
...
git-tree-sha1 = "c004ce0d7c304681ab5434fb89fc475ca2d71b82"
